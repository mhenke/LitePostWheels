			...
			for (loc.i=1; loc.i <= loc.iEnd; loc.i++)
			{
				...
				for (loc.j=1; loc.j <= loc.jEnd; loc.j++)
				{
					...
					if (StructKeyExists(loc.classData, "class"))
						loc.modelastName = loc.classData.class;
					else if (StructKeyExists(loc.classData, "name"))
						loc.modelastName = loc.classData.name;
	
					// create a struct for this model unless it already exists
					if (!StructKeyExists(loc.addedPropertiesByModel, loc.modelastName))
						loc.addedPropertiesByModel[loc.modelastName] = "";
	
					// if we find the property in this model and it's not already added we go ahead and add it to the select clause
					if ((ListFindNoCase(loc.classData.propertyList, loc.iItem) || ListFindNoCase(loc.classData.calculatedPropertyList, loc.iItem)) && !ListFindNoCase(loc.addedPropertiesByModel[loc.modelastName], loc.iItem))
					{
						...
						loc.addedPropertiesByModel[loc.modelastName] = ListAppend(loc.addedPropertiesByModel[loc.modelastName], loc.iItem);
						break;
					}
				}
				if (Len(loc.toAppend))
			...
			if (Len(arguments.include))
			{
				...
				for (loc.i=1; loc.i <= loc.iEnd; loc.i++)
				{
					...
					else
					{
						...
						loc.newProperty = loc.modelastName & loc.property;
