...
<cffunction name="property" returntype="void" access="public" output="false"
	...
	'
		<!--- In models/User.cfc --->
		<cffunction name="init">
			<!--- Tell Wheels that when we are referring to `firstName` in the CFML code it should translate to the `STR_USERS_firstName` column when interacting with the database instead of the default (which would be the `firstname` column) --->
  			<cfset property(name="firstName", column="STR_USERS_firstName")>
		</cffunction>
	'
